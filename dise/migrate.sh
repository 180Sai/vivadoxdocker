#!/bin/bash
# dise/migrate.sh — ISE workspace sync (host ↔ Docker volume)

HOST_DIR="${1:-/mnt/host-workspace-ise}"
CONTAINER_DIR="/home/guest/workspace-ise"
...
# Smart excludes for Xilinx ISE (ignore simulation and temp folders)
RSYNC_EXCLUDES=(
...
detect_and_archive() {
    ...
    # rsync --dry-run to see if container differs from host
...
if [[ "$*" == *"--migrate"* ]]; then
    ...
    ask_confirm "Migrate ${HOST_DIR} → container" "${AUTO_MIGRATE}" || exit 0
    echo "[sync] Copying ${HOST_DIR} → ${CONTAINER_DIR}..."
...
if [[ "$*" == *"--restore"* ]]; then
    ...
    ask_confirm "Restore ${CONTAINER_DIR} → ${HOST_DIR}" "${AUTO_RESTORE}" || { sleep 1; exit 0; }
    echo "[sync] Copying ${CONTAINER_DIR} → ${HOST_DIR}..."
